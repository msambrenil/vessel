...
| **Pulso Enviado (`playSignalSent` / `playPulseSent`)** | 75 Hz | 0.30s | Doble pulso afirmativo de confirmación sub-bass. |
...
1. `PULSE_TRANSMITTED` *(canónico)* / `SIGNAL_TRANSMITTED` *(alias legacy)*:
   - Payload: `{ targetProfileId, intensity, timestamp }`.
   - Efecto: Registra el pulso enviado y activa feedback háptico/acústico (75 Hz). Unificado bajo Regla de Negocio #8.
2. `PULSE_RETURNED`:
...
7. `APP_SETTINGS_SYNCED`:
   ...
   - Efecto: Persiste preferencias globales en almacenamiento local cifrado. *Nota de Privacidad:* Los registros íntimos del `Date Diary`, botiquín Doxy-PEP y contactos de auxilio quedan estrictamente excluidos de la sincronización en nube y permanecen 100% en almacenamiento Local-First.
